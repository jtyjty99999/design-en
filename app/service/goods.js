...
module.exports = app => {
  class MonthServer extends app.Service {
    ...
    * find(id) {
    ...
    // 获取某一类的project
    * search(type) {
      const article = yield app.mysql.query('select  id,title,  m_pic, content, type, price from design_goods where type like \'%' + type + '%\' and deleted = 0 order by timestamp desc', [ type ]);
      return article;
    }
